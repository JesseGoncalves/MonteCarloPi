...
double trials(boost::uint64_t numTrials,
   boost::uint64_t
   ...
   boost::uint64_t packetsPerNode)    // calculates estimate of pi
{
   ...
   hpx::parallel::for_loop(hpx::parallel::par, 0, packetsPerNode,
       hpx::parallel::reduction_plus(piEstimate),
       [&](boost::uint64_t k, double& piEstimate) {
           ...
           {
               double x = rand01();
               double y = rand01();    // generate random x and y coordinates
...
int hpx_main(boost::program_options::variables_map& vm)
{
   ...
   {    // repeat 10 times
       ...
       hpx::future<double> piEstimate = hpx::async(runTrials, hpx::find_here(),
           numTrials, packetsPerNode);    // estimate of pi from each thread

       if (hpx::get_locality_id() != 0)
       {
           hpx::lcos::gather_there(gather_basename, std::move(piEstimate), j)
               .wait();    // pass estimate to node 0
       }
       else
       {
           hpx::future<std::vector<double>> resultFuture =
               hpx::lcos::gather_here(gather_basename, std::move(piEstimate),
           ...
           std::ofstream resultFile("HPXdist2Data.csv",
               std::ios::out | std::ios::app);    // write results to .csv file

           resultFile << result << ", " << numTrials << ", " << numPackets
                      << ", " << numNodes << ", " << execTime.count()
                      << std::endl;
       }
   }
   return hpx::finalize();
