...
int main()
{
  static unsigned long long numTrials = 100000000;  // number of trials for monte carlo simulation
  ...
  for (numThreads = 1; numThreads < 17; ++numThreads) {  // iterate for different numbers of threads
    ...
    for (i = 0; i < 10; ++i) {  // repeat 10 times at each number of threads
      ...
      std::ofstream resultFile("twisterData.csv", std::ios::out | std::ios::app);
